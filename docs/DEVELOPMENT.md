...
| Область | Что |
|---------|-----|
| **Лента** | Периоды: все 7 дн. / свежие / архив; **📜 История** (30 дн., поиск); без двойного re-check категории |
| **Парсер** | Категория **handyman** (разнорабочий/клининг); «по договорённости» в ingest и карточке; тюнинг gate |
| **UX** | 🟡 Не подходит, эмодзи на карточке, жалоба «не та категория»; быстрее отклик кнопок (callback) |
| **Premium** | **📡 Предложить канал** — заявка → модерация админом (`chat_suggestions`) |
| **Тесты** | 297 pytest; `/help` обновлён |
...
- [ ] **Подписка на канал @promostaff_agency_job** — `getChatMember` + экран «Подписаться» (мягко/жёстко TBD) → [`BACKLOG_REFERRAL_AND_ENGAGEMENT.md`](BACKLOG_REFERRAL_AND_ENGAGEMENT.md) §5.
...
| Дата | Что добавлено |
|------|----------------|
| 2026-06-10 | Лента/история, handyman, vacancy_rate, Premium «Предложить канал», help, TECH_ANNOUNCEMENT |
