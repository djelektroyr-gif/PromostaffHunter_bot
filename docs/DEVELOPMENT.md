...
| 📋 Категории | Просмотр и изменение категорий (один экран, inline) |
| 📍 Мои районы | Фильтр метро (Premium) |
| 💎 Подписка | Тариф, продление, оплата |
| 📞 Мои контакты | Профиль |
| 📖 Как пользоваться | Инструкция (/help) — шаги и команды |
| ❌ Отписаться | `is_active = 0` |
| ❓ Поддержка | FSM → обращение админу |

> Старые кнопки «📋 Мои категории» / «✏️ Изменить категории» остаются совместимыми — ведут в тот же экран.

### UX-пакет 1–3 (2026-06-04) — реализовано

1. **📨 Мои отклики** — пагинация, статус вакансии (активна/закрыта), ссылки.
2. **📋 Категории** — одна reply-кнопка + inline-выбор (без дубля «Изменить»).
3. **Продление Premium** — кнопки для активных подписчиков; `/setplan` и выдача админом **добавляют** дни к `paid_until`, а не сбрасывают с «сегодня».

### UX — следующий этап (согласовано, не в этом PR)

- [ ] **👤 Профиль** — объединить контакты, фото, анкету.
- [ ] Счётчик на кнопке ленты вакансий.
- [ ] Напоминание за 3 дня до конца Premium (cron).
- [ ] `set_my_commands` (`/start`, `/menu`, `/help`).
