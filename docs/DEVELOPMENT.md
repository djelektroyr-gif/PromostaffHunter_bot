...
**Проверка на проде:** в логах `⚡ Парсер групп: новое сообщение` должно быть близко ко времени поста; если только `🔄 Плановая проверка` — realtime не ловит чат (см. «в мониторинге» в статистике).
