...
def format_premium_renewal_reminder(
    *,
    ...
    if trial_used and days_left <= 7:
        ...
        tail = (
            "После окончания push отключится — останется лента «🔍 Посмотреть новые вакансии».\n"
            f"Оформить Premium: *{SUBSCRIPTION_PRICE_RUB} ₽/мес* — кнопка ниже или «💎 Подписка» в меню."
        )
    else:
        heading = "⏳ *Premium скоро закончится*"
        tail = (
            "Push по категориям и фильтр метро перестанут работать.\n"
